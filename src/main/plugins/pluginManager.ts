...
export class PluginManager extends EventEmitter {
    ...
    async loadPlugin(pluginId: string): Promise<boolean> {
        ...
        try {
            ...
            // Load frontend entrypoint if specified (support .js and .ts for dev)
            const mainConfig = (plugin.manifest as any).frontend;
            if (mainConfig?.entrypoint) {
                ...
                if (fs.existsSync(entrypointPath)) {
                    console.log(`[PluginManager] Loading frontend entrypoint for ${pluginId}: ${entrypointPath}`);
                    try {
                        ...
                        // Store module and call onStartup hook
                        plugin.module = module;
                        if (typeof module.onStartup === 'function') {
                            console.log(`[PluginManager] Calling onStartup hook for plugin ${pluginId}`);
                            try {
                                const result = module.onStartup({
                                    mainWindow: this.mainWindow,
                                    windowManager: this.windowManager
                                });
                                ...
                                console.error(`[PluginManager] Error in onStartup for ${pluginId}:`, e);
                            }
                        }
                    } catch (e) {
    ...
    async unloadPlugin(pluginId: string): Promise<boolean> {
        ...
        try {
            ...
            // Call onStop hook if exists
            if (plugin.module && typeof plugin.module.onStop === 'function') {
                console.log(`[PluginManager] Calling onStop hook for plugin ${pluginId}`);
                try {
                    const result = plugin.module.onStop({
                        mainWindow: this.mainWindow,
                        windowManager: this.windowManager
                    });
                    if (result instanceof Promise) {
                    ...
                    console.error(`[PluginManager] Error in onStop for ${pluginId}:`, e);
                }
            }
