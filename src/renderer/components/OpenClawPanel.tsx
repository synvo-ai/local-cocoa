...
export function OpenClawPanel() {
    ...
    const loadStatus = useCallback(async () => {
        ...
        const key = localKey ?? (await window.api?.getLocalKey?.()) ?? null;
        if (!key) {
            setError('Local Cocoa API key unavailable.');
            setLoading(false);
            return;
        ...
        setError(null);
        try {
            const data = await fetchJson<TelegramStatus>(key, '/plugins/telegram/status');
            setStatus(data);
        } catch (err) {
    ...
    const loadMessages = useCallback(async () => {
        ...
        try {
            const data = await fetchJson<{ messages: TelegramMessage[] }>(
                key,
                '/plugins/telegram/messages?limit=100'
            );
    ...
    const handleRestart = async () => {
        ...
        setError(null);
        try {
            await fetchJson<ActionResult>(localKey, '/plugins/telegram/restart', { method: 'POST' });
            await loadStatus();
            await loadMessages();
        } catch (err) {
